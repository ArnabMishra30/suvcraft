...
function BulkSection({ title, blocks }) {
  return (
    <section>
      <h3 className="text-base font-semibold text-slate-900 dark:text-white mb-3">{title}</h3>
      <div className="space-y-4">
        {blocks.map((b) => (
          <div key={b.heading} className="rounded-lg border border-slate-200 dark:border-slate-800 overflow-hidden">
            <div className="px-3 py-2 bg-slate-50 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800">
              <h4 className="text-sm font-semibold text-slate-800 dark:text-slate-200">{b.heading}</h4>
            </div>
            <ol className="divide-y divide-slate-200 dark:divide-slate-800">
              {b.items.map((it, i) => {
                const [field, desc, req] = it;
                return (
                  <li key={i} className="px-3 py-2 flex items-start gap-2 text-sm">
                    <span className="text-slate-400 tabular-nums w-5 flex-shrink-0">{i + 1}.</span>
                    <code className="font-mono text-indigo-600 dark:text-indigo-400 flex-shrink-0">{field}</code>
                    <span className="text-slate-400">—</span>
                    <span className="flex-1 text-slate-600 dark:text-slate-400">{desc}</span>
