...
import CityGroupFormModal from './city-group-form-modal';

export function AddCityGroupButton({ cities = [] }) {
  ...
  return (
    <>
      ...
      <CityGroupFormModal open={open} onClose={() => setOpen(false)} cities={cities} />
    </>
  );
}

export function CityGroupRowActions({ row, cities = [] }) {
  ...
  async function openEdit() {
    ...
    try {
      const res = await fetch(`/api/admin/city-groups/${row.id}`);
  ...
  async function onDelete() {
    setMenu(false);
    if (!confirm(`Delete cities group "${row.group_name}"?`)) return;
    setBusy(true);
    try {
      const res = await fetch(`/api/admin/city-groups/${row.id}`, { method: 'DELETE' });
      const json = await res.json();
  ...
  return (
    <>
      ...
      <CityGroupFormModal open={editOpen} onClose={() => { setEditOpen(false); setInitial(null); }} initial={initial} cities={cities} />
    </>
  );
}