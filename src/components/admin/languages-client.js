  ...
  return (
    <>
      <button type="button" onClick={() => setOpen(true)}
        className="inline-flex items-center justify-center gap-2 rounded-lg bg-indigo-50 dark:bg-indigo-950/40 hover:bg-indigo-100 dark:hover:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300 px-4 py-2 text-sm font-semibold border border-indigo-200 dark:border-indigo-900 w-full">
      ...
      <LanguageFormModal open={open} onClose={() => setOpen(false)} />
    </>
  );
}

export function LanguageRowActions({ row }) {
  ...
  async function setDefault() {
    ...
    try {
      const res = await fetch(`/api/admin/languages/${row.id}`, {
  ...
  async function onDelete() {
    setMenu(false);
    if (!confirm(`Delete language "${row.language}"?`)) return;
    setBusy(true);
    try {
  ...
  async function onUpload(e) {
    ...
    try {
      ...
      fd.append('file', file);
      const res = await fetch(`/api/admin/languages/${row.id}/upload`, { method: 'POST', body: fd });
      const json = await res.json();
      alert(json.error ? json.message : (json.message || 'Uploaded.'));
  ...
  return (
    <>
      ...
      <input ref={fileRef} type="file" accept=".json,.csv,application/json" className="hidden" onChange={onUpload} />
      {menu && pos && (
        <div ref={menuRef} style={{ position: 'fixed', top: pos.top, left: pos.left, width: 200 }}
          ...
          <button type="button" onClick={() => fileRef.current?.click()}
            className="w-full flex items-center gap-2 px-3 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800">
            <svg className="w-4 h-4 text-emerald-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M7 16a4 4 0 01-.88-7.903A5 5 0 0115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" /></svg>
            Upload File
          </button>
          {isDefault ? (
            <div className="w-full flex items-center gap-2 px-3 py-2 text-sm text-slate-400 cursor-not-allowed">
      ...
      <LanguageFormModal open={editOpen} onClose={() => setEditOpen(false)} initial={row} />
    </>
  );
}