...
export function AddOfferButton({ categories = [], products = [] }) {
  ...
  return (
    <>
      ...
      <OfferFormModal open={open} onClose={() => setOpen(false)} categories={categories} products={products} />
    </>
  );
}

export function OfferRowActions({ row, categories = [], products = [] }) {
  ...
  async function onDelete() {
    setMenu(false);
    if (!confirm(`Delete offer #${row.id}?`)) return;
    setBusy(true);
    try {
  ...
  return (
    <>
      ...
      <OfferFormModal open={editOpen} onClose={() => setEditOpen(false)} initial={row} categories={categories} products={products} />
    </>
  );
}