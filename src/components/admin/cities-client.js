...
export function AddCityButton() {
  ...
  return (
    <>
      ...
      <CityFormModal open={open} onClose={() => setOpen(false)} />
    </>
  );
}

export function CityRowActions({ row }) {
  ...
  async function onDelete() {
    ...
    setBusy(true);
    try {
      const res = await fetch(`/api/admin/cities/${row.id}`, { method: 'DELETE' });
  ...
  return (
    <>
      ...
      <CityFormModal open={editOpen} onClose={() => setEditOpen(false)} initial={row} />
    </>
  );
}