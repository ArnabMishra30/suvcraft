...
export default function BrandFormModal({ open, onClose, initial }) {
  ...
  async function save() {
    ...
    if (!image.trim()) { setErr('Main image is required.'); return; }
    setBusy(true); setErr('');
    try {
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({ name: name.trim(), image: image.trim(), status: Number(status) }),
      });
  ...
  return (
    <Modal
      ...
      footer={<>
        ...
        <button type="button" onClick={save} disabled={busy} className="px-4 py-2 rounded-md text-sm bg-indigo-600 hover:bg-indigo-500 text-white disabled:opacity-60">{busy ? 'Saving…' : (isEdit ? 'Update Brand' : 'Add Brand')}</button>
      </>}
    >
      <div className="space-y-4">
        <div>
          <label className={labelCls}>Name <span className="text-red-500">*</span></label>
          <input autoFocus value={name} onChange={(e) => setName(e.target.value)} className={inputCls} placeholder="Add Brand Name" />
        </div>
        <MediaPickerCard
          title="Main Image *"
