...
export default function OfferFormModal({ open, onClose, initial, categories = [], products = [] }) {
  ...
  useEffect(() => {
    ...
    setLink(initial?.type === 'offer_url' ? (initial?.link || '') : '');
    setErr('');
  }, [open, initial]);

  async function save() {
    if (!type) { setErr('Type is required.'); return; }
    if (!image) { setErr('Offer image is required.'); return; }
    ...
    if (type === 'offer_url' && !link.trim()) { setErr('Offer URL is required.'); return; }
  ...
  return (
    <Modal open={open} onClose={onClose} title={isEdit ? 'Edit Offer' : 'Add Offer'} size="lg"
      footer={<>
        ...
        <button type="button" onClick={save} disabled={busy} className="px-4 py-2 rounded-md text-sm bg-indigo-600 hover:bg-indigo-500 text-white disabled:opacity-60">{busy ? 'Saving…' : (isEdit ? 'Update Offer' : 'Add Offer')}</button>
      </>}>
      <div className="space-y-4">
        ...
        {type === 'offer_url' && (
          <div>
            <label className={labelCls}>Offer URL <span className="text-red-500">*</span></label>
