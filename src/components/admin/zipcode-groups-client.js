...
import ZipcodeGroupFormModal from './zipcode-group-form-modal';

export function AddZipcodeGroupButton({ zipcodes = [] }) {
  ...
  return (
    <>
      ...
      <ZipcodeGroupFormModal open={open} onClose={() => setOpen(false)} zipcodes={zipcodes} />
    </>
  );
}

export function ZipcodeGroupRowActions({ row, zipcodes = [] }) {
  ...
  async function openEdit() {
    ...
    try {
      const res = await fetch(`/api/admin/zipcode-groups/${row.id}`);
  ...
  async function onDelete() {
    setMenu(false);
    if (!confirm(`Delete zipcode group "${row.group_name}"?`)) return;
    setBusy(true);
    try {
      const res = await fetch(`/api/admin/zipcode-groups/${row.id}`, { method: 'DELETE' });
      const json = await res.json();
  ...
  return (
    <>
      ...
      <ZipcodeGroupFormModal open={editOpen} onClose={() => { setEditOpen(false); setInitial(null); }} initial={initial} zipcodes={zipcodes} />
    </>
  );
}