  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
      <div className="px-5 py-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
        ...
        <Link href="/api/admin/products/export" className="text-sm text-indigo-600 hover:text-indigo-500 inline-flex items-center gap-1" onClick={(e) => e.preventDefault()} title="Coming soon">
      ...
      <form onSubmit={onSubmit} className="p-5 space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
          <div>
            <label htmlFor="type" className={labelCls}>Type <span className="text-slate-400">[upload/update]</span> <span className="text-red-500">*</span></label>
            <select id="type" value={type} onChange={(e) => setType(e.target.value)} className={inputCls}>
              <option value="upload">Upload (create new products)</option>
              <option value="update">Update (modify existing)</option>
            </select>
          </div>
          <div className="sm:col-span-2">
            <label htmlFor="file" className={labelCls}>File <span className="text-red-500">*</span></label>
        ...
        {err && (
          <div role="alert" className="rounded-lg bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900 px-3 py-2 text-sm text-red-700 dark:text-red-300">
            {err}
          </div>
        )}
        ...
        {result && (
          <div role="status" className="rounded-lg bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-900 px-3 py-3 text-sm text-emerald-800 dark:text-emerald-200 space-y-2">
            <div className="font-medium">{result.created} of {result.total} product(s) created.</div>
