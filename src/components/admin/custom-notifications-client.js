...
import CustomNotificationFormModal from './custom-notification-form-modal';

export function AddCustomMessageButton() {
  ...
  return (
    <>
      ...
      <CustomNotificationFormModal open={open} onClose={() => setOpen(false)} />
    </>
  );
}

export function CustomMessageRowActions({ row }) {
  const router = useRouter();
  const [editOpen, setEditOpen] = useState(false);
  const [busy, setBusy] = useState(false);

  async function onDelete() {
    if (!confirm(`Delete custom message "${row.title}"?`)) return;
    setBusy(true);
    try {
      const res = await fetch(`/api/admin/custom-notifications/${row.id}`, { method: 'DELETE' });
  ...
  return (
    <>
      ...
      <CustomNotificationFormModal open={editOpen} onClose={() => setEditOpen(false)} initial={row} />
    </>
  );
}