...
export default function ZipcodeGroupFormModal({ open, onClose, initial, zipcodes = [] }) {
  ...
  useEffect(() => {
    ...
    setIds(Array.isArray(initial?.zipcode_ids) ? initial.zipcode_ids.map(String) : []);
  ...
  async function save() {
    ...
    if (!ids.length) { setErr('Select at least one deliverable zipcode.'); return; }
    setBusy(true); setErr('');
    try {
      const url = isEdit ? `/api/admin/zipcode-groups/${initial.id}` : '/api/admin/zipcode-groups';
      const method = isEdit ? 'PUT' : 'POST';
      const res = await fetch(url, {
        method, headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          group_name: name.trim(),
          zipcode_ids: ids.map(Number),
  ...
  return (
    <Modal open={open} onClose={onClose} title={isEdit ? 'Edit Zipcode Group' : 'Add Zipcode Group'} size="md"
      footer={<>
        <button type="button" onClick={save} disabled={busy} className="px-4 py-2 rounded-md text-sm bg-indigo-600 hover:bg-indigo-500 text-white disabled:opacity-60">{busy ? 'Saving…' : (isEdit ? 'Update Zipcode Group' : 'Add Zipcode Group')}</button>
      ...
      <div className="space-y-4">
        ...
        <div>
          <label className={labelCls}>Deliverable Zipcodes <span className="text-red-500">*</span></label>
          <MultiSearchableSelect values={ids} onChange={setIds} options={zipcodes}
            placeholder="Search Zipcode…"
            getLabel={(z) => z.zipcode}
            getSubLabel={(z) => z.city_name || ''} />
        </div>
        <div>
