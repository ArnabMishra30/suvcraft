...
import BlogCategoryFormModal from './blog-category-form-modal';

export function AddBlogCategoryButton() {
  ...
  return (
    <>
      ...
      <BlogCategoryFormModal open={open} onClose={() => setOpen(false)} />
    </>
  );
}

export function BlogCategoryStatusToggle({ row }) {
...
export function BlogCategoryRowActions({ row }) {
  const router = useRouter();
  ...
  async function onDelete() {
    setMenu(false);
    if (!confirm(`Delete blog category "${row.name}"?`)) return;
    setBusy(true);
    try {
      const res = await fetch(`/api/admin/blog-categories/${row.id}`, { method: 'DELETE' });
      const json = await res.json();
      if (json.error) alert(json.message);
      else router.refresh();
    } finally { setBusy(false); }
  ...
  return (
    <>
      ...
      <BlogCategoryFormModal open={editOpen} onClose={() => setEditOpen(false)} initial={row} />
    </>
  );
}