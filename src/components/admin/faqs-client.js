...
export function AddFaqButton() {
  ...
  return (
    <>
      ...
      <FaqFormModal open={open} onClose={() => setOpen(false)} />
    </>
  );
}

export function FaqStatusToggle({ row }) {
...
export function FaqRowActions({ row }) {
  const router = useRouter();
  ...
  return (
    <>
      ...
      <FaqFormModal open={editOpen} onClose={() => setEditOpen(false)} initial={row} />
    </>
  );
}