...
export function AddAttributeButton({ attributeSets }) {
  ...
  return (
    <>
      ...
      <AttributeFormModal open={open} onClose={() => setOpen(false)} attributeSets={attributeSets} />
    </>
  );
}

export function AttributeRowActions({ row, attributeSets }) {
  ...
  async function onDelete() {
    if (!confirm(`Delete attribute "${row.name}"? Its values will become orphaned.`)) return;
    setBusy(true); setErr('');
    try {
      const res = await fetch(`/api/admin/attributes/${row.id}`, { method: 'DELETE' });
      const json = await res.json();
  ...
  return (
    <>
      ...
      <AttributeFormModal open={editOpen} onClose={() => setEditOpen(false)} initial={row} attributeSets={attributeSets} />
    </>
  );
}