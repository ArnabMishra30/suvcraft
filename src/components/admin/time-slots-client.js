...
export function AddTimeSlotButton() {
  ...
  return (
    <>
      ...
      <TimeSlotFormModal open={open} onClose={() => setOpen(false)} />
    </>
  );
}

export function TimeSlotRowActions({ row }) {
  const router = useRouter();
  const [editOpen, setEditOpen] = useState(false);
  const [busy, setBusy] = useState(false);

  async function onDelete() {
    if (!confirm(`Delete time slot "${row.title}"?`)) return;
    setBusy(true);
    try {
      const res = await fetch(`/api/admin/time-slots/${row.id}`, { method: 'DELETE' });
  ...
  return (
    <>
      ...
      <TimeSlotFormModal open={editOpen} onClose={() => setEditOpen(false)} initial={row} />
    </>
  );
}