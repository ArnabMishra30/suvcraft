...
import TicketTypeFormModal from './ticket-type-form-modal';
...
export function AddTicketTypeButton() {
  ...
  return (
    <>
      ...
      <TicketTypeFormModal open={open} onClose={() => setOpen(false)} />
    </>
  );
}

export function TicketTypeFilter({ types = [] }) {
  const router = useRouter();
  const pathname = usePathname();
  const params = useSearchParams();
  const [typeId, setTypeId] = useState(params.get('typeId') || '');

  useEffect(() => { setTypeId(params.get('typeId') || ''); }, [params]);

  function setAndApply(v) {
    setTypeId(v);
    const sp = new URLSearchParams(params);
    if (v) sp.set('typeId', v); else sp.delete('typeId');
    sp.delete('page');
  ...
  return (
    <div className="max-w-md">
      <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Filter By Ticket Type</label>
      <SearchableSelect value={typeId} onChange={setAndApply} options={types} placeholder="Select Ticket Type" />
    </div>
  );
}

export function TicketTypeRowActions({ row }) {
  const router = useRouter();
  ...
  async function onDelete() {
    setMenu(false);
    if (!confirm(`Delete ticket type "${row.title}"?`)) return;
    setBusy(true);
    try {
      const res = await fetch(`/api/admin/ticket-types/${row.id}`, { method: 'DELETE' });
  ...
  return (
    <>
      ...
      <TicketTypeFormModal open={editOpen} onClose={() => setEditOpen(false)} initial={row} />
    </>
  );
}