...
export default function CategoryBulkForm() {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      fd.append('file', file);
      fd.append('type', type);
      const res = await fetch('/api/admin/categories/bulk-upload', { method: 'POST', body: fd });
      const json = await res.json();
      if (json.error) { setErr(json.message || 'Upload failed.'); return; }
      setResult({ ...json.data, kind: type });
    } catch {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
          <div>
            ...
            <select id="type" value={type} onChange={(e) => setType(e.target.value)} className={inputCls}>
              <option value="upload">Upload (create new categories)</option>
        ...
        {result && (
          <div role="status" className="rounded-lg bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-900 px-3 py-3 text-sm text-emerald-800 dark:text-emerald-200 space-y-2">
            <div className="font-medium">
              {result.kind === 'upload'
                ? `${result.created} of ${result.total} categor${result.total === 1 ? 'y' : 'ies'} created.`
                : `${result.updated} of ${result.total} categor${result.total === 1 ? 'y' : 'ies'} updated.`}
            </div>
