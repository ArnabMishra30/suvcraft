...
import AttributeSetFormModal from './attribute-set-form-modal';

export function AddAttributeSetButton() {
  ...
  return (
    <>
      ...
      <AttributeSetFormModal open={open} onClose={() => setOpen(false)} />
    </>
  );
}

export function AttributeSetRowActions({ row }) {
  ...
  async function toggleStatus() {
    setBusy(true); setErr('');
    try {
      const res = await fetch(`/api/admin/attribute-sets/${row.id}`, {
        method: 'PATCH',
  ...
  async function onDelete() {
    if (!confirm(`Delete attribute set "${row.name}"? Attributes inside it will become orphaned.`)) return;
    setBusy(true); setErr('');
    try {
      const res = await fetch(`/api/admin/attribute-sets/${row.id}`, { method: 'DELETE' });
      const json = await res.json();
  ...
  return (
    <>
      ...
      <AttributeSetFormModal open={editOpen} onClose={() => setEditOpen(false)} initial={row} />
    </>
  );
}