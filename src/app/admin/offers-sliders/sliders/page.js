...
import {
  ...
  SLIDER_TYPE_LABEL,
} from '@/lib/repos/slider';
import TableToolbar from '@/components/admin/table-toolbar';
import ProductImage from '@/components/admin/product-image';
import SliderFilters from '@/components/admin/slider-filters';
import { AddSliderButton, SliderRowActions } from '@/components/admin/sliders-client';
...
const TYPE_BADGE = {
  ...
  sliderurl: 'bg-violet-100 text-violet-700 dark:bg-violet-950 dark:text-violet-300',
};

export default async function SlidersPage({ searchParams }) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        <div>
          <h1 className="text-2xl sm:text-3xl font-semibold text-slate-900 dark:text-white">Sliders For Add-on Offers and other benefits</h1>
          <p className="mt-1 text-sm text-slate-500">Banners shown on the storefront home and category pages.</p>
        </div>
        <nav className="text-xs text-slate-500">
          <Link href="/admin" className="hover:text-slate-700 dark:hover:text-slate-300">Home</Link>
          <span className="mx-1.5">›</span>
          <span className="text-slate-700 dark:text-slate-300">Manage Slider</span>
        </nav>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
        <div className="px-5 py-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between gap-3">
          ...
          <AddSliderButton categories={categories} products={products} />
        </div>

        <div data-sliders-table>
          <TableToolbar title="" columns={COLUMNS} defaultVisible={DEFAULT_VISIBLE} tableAttr="data-sliders-table" storageKey="admin.sliders.cols.v1" exportFilename="sliders" />
                  ...
                  <tr key={s.id} className="hover:bg-slate-50 dark:hover:bg-slate-950/50">
                    ...
                    <td data-col="type" className="px-5 py-3 text-center">
                      <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${TYPE_BADGE[s.type] || ''}`}>
                        {SLIDER_TYPE_LABEL[s.type] || s.type}
                      </span>
                    </td>
                    <td data-col="type_id" className="px-5 py-3 text-center text-slate-700 dark:text-slate-300">{s.type_id || '—'}</td>
                    <td data-col="name" className="px-5 py-3 font-medium text-slate-900 dark:text-white max-w-[18rem]">
                      <div className="line-clamp-2" title={s.name || ''}>{s.name || '—'}</div>
                    </td>
                    <td data-col="image" className="px-5 py-3 text-center">
                      {s.image ? <div className="inline-block"><ProductImage src={s.image} alt={`Slider ${s.id}`} /></div> : <span className="text-xs text-slate-400">—</span>}
                    </td>
                    <td data-col="link" className="px-5 py-3 text-slate-700 dark:text-slate-300 max-w-[18rem]">
                      {s.link
                        ? <a href={s.link} target="_blank" rel="noreferrer" className="text-indigo-600 hover:text-indigo-500 truncate block" title={s.link}>{s.link}</a>
                        : <span className="text-slate-500">—</span>}
                    </td>
                    <td data-col="action" className="px-5 py-3 text-center">
                      <SliderRowActions row={s} categories={categories} products={products} />
                    </td>
                  </tr>
                ))}
              </tbody>
