...
import BrandBulkForm from '@/components/admin/brand-bulk-form';

export const dynamic = 'force-dynamic';

export default function BrandBulkUploadPage() {
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        <div>
          <h1 className="text-2xl sm:text-3xl font-semibold text-slate-900 dark:text-white">Brand bulk upload</h1>
        </div>
        <nav className="text-xs text-slate-500">
          <Link href="/admin" className="hover:text-slate-700 dark:hover:text-slate-300">Home</Link>
          <span className="mx-1.5">›</span>
          <Link href="/admin/brands" className="hover:text-slate-700 dark:hover:text-slate-300">Brands</Link>
          <span className="mx-1.5">›</span>
        ...
        <div className="flex gap-3">
          ...
          <div className="text-sm text-blue-900 dark:text-blue-200">
            ...
            <ul className="list-disc pl-5 space-y-0.5 text-blue-800 dark:text-blue-300">
              ...
              <li>For adding bulk Brand file should be .csv format</li>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
          <DownloadBtn href="/api/admin/brands/sample?kind=upload" label="Bulk upload template" />
          <DownloadBtn href="/api/admin/brands/sample?kind=update" label="Bulk update template" />
          <DownloadBtn href="/api/admin/brands/export" label="Bulk Download file" />
        </div>
      </div>
    </div>
  );
...
function DownloadBtn({ href, label }) {
  return (
    <a href={href} className="flex items-center justify-center gap-2 rounded-lg border border-blue-200/50 dark:border-blue-900/50 bg-blue-50/40 dark:bg-blue-950/20 hover:bg-blue-50 dark:hover:bg-blue-950/40 text-blue-700 dark:text-blue-300 px-4 py-3 text-sm font-medium transition">
