...
export default async function PolicyEditorPage({ params }) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        ...
        <nav className="text-xs text-slate-500">
          ...
          <Link href="/admin/policies" className="hover:text-slate-700 dark:hover:text-slate-300">Policies</Link>
          <span className="mx-1.5">›</span>
          <span className="text-slate-700 dark:text-slate-300">{policy.title}</span>
        </nav>
      </div>
