...
import { listZipcodeGroups, listZipcodesForSelect } from '@/lib/repos/location';
import { getSettings } from '@/lib/settings';
import { formatCurrency } from '@/lib/format';
import TableToolbar from '@/components/admin/table-toolbar';
import { AddZipcodeGroupButton, ZipcodeGroupRowActions } from '@/components/admin/zipcode-groups-client';
...
const COLUMNS = [
  ...
  { key: 'zipcodes', label: 'Zipcodes', defaultVisible: true },
...
export default async function ZipcodesGroupPage({ searchParams }) {
  ...
  const [{ rows, total, totalPages, page }, zipcodes, sys] = await Promise.all([
    listZipcodeGroups(filters),
    listZipcodesForSelect(),
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        <div>
          <h1 className="text-2xl sm:text-3xl font-semibold text-slate-900 dark:text-white">Manage Zipcodes Group</h1>
          <p className="mt-1 text-sm text-slate-500">Bundle multiple zipcodes under a single delivery rule.</p>
        </div>
        <nav className="text-xs text-slate-500">
          ...
          <span className="mx-1.5">›</span>
          <span className="text-slate-700 dark:text-slate-300">Zipcodes Group</span>
        </nav>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
        <div className="px-5 py-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between gap-3">
          <h2 className="text-sm font-semibold text-slate-900 dark:text-white inline-flex items-center gap-2">
            ...
            Zipcodes Group Details
          </h2>
          <AddZipcodeGroupButton zipcodes={zipcodes} />
        </div>

        <div data-zipcode-groups-table>
          <TableToolbar title="" columns={COLUMNS} defaultVisible={DEFAULT_VISIBLE} tableAttr="data-zipcode-groups-table" storageKey="admin.zipcode-groups.cols.v1" exportFilename="zipcodes-group" />
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50 dark:bg-slate-950 text-left text-xs uppercase tracking-wide text-slate-500">
                <tr>
                  ...
                  <th data-col="zipcodes" className="px-5 py-3">Zipcodes</th>
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                ...
                {rows.map((r) => (
                  <tr key={r.id} className="hover:bg-slate-50 dark:hover:bg-slate-950/50">
                    ...
                    <td data-col="zipcodes" className="px-5 py-3 text-slate-700 dark:text-slate-300 max-w-[28rem]">
                      <div className="line-clamp-2" title={r.zipcodes || ''}>{r.zipcodes || '—'}</div>
                    </td>
                    ...
                    <td data-col="actions" className="px-5 py-3 text-center"><ZipcodeGroupRowActions row={r} zipcodes={zipcodes} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
