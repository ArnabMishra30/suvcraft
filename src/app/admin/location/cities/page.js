...
import { listCities } from '@/lib/repos/location';
import { getSettings } from '@/lib/settings';
import { formatCurrency } from '@/lib/format';
import TableToolbar from '@/components/admin/table-toolbar';
import { AddCityButton, CityRowActions } from '@/components/admin/cities-client';
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        <div>
          <h1 className="text-2xl sm:text-3xl font-semibold text-slate-900 dark:text-white">Manage Cities</h1>
          <p className="mt-1 text-sm text-slate-500">Cities and their default delivery rules.</p>
        </div>
        <nav className="text-xs text-slate-500">
          ...
          <span className="mx-1.5">›</span>
          <span className="text-slate-700 dark:text-slate-300">Cities</span>
        </nav>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
        ...
        <div data-cities-table>
          <TableToolbar title="" columns={COLUMNS} defaultVisible={DEFAULT_VISIBLE} tableAttr="data-cities-table" storageKey="admin.cities.cols.v1" exportFilename="cities" />
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                ...
                {rows.map((r) => (
                  <tr key={r.id} className="hover:bg-slate-50 dark:hover:bg-slate-950/50">
                    ...
                    <td data-col="actions" className="px-5 py-3 text-center"><CityRowActions row={r} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
